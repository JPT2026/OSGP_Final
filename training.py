...
# === Load band values at training point locations ===
...
# === Use actual landcover labels ===
if 'classname' not in gdf_clean.columns:
    raise ValueError("Shapefile must include a 'classname' column with true labels including 'Ice'.")

# === Oversample Ice class to balance data ===
...
# === Model pipeline ===
pipe = Pipeline([
    ('pca', PCA(n_components=2)),  # Optional: remove if full feature space preferred
    ('rf', RandomForestClassifier(n_estimators=100, class_weight='balanced', random_state=42))
])
pipe.fit(X_train, y_train)

# === Read and stack all raster bands ===
...
stacked = np.stack(all_bands)  # (bands, height, width)
reshaped = stacked.reshape(len(all_bands), -1).T  # (n_pixels, n_bands)

# === 🛠 FIX: Create DataFrame for prediction ===
reshaped_df = pd.DataFrame(reshaped, columns=band_cols)

# === Predict landcover classes ===
preds = pipe.predict(reshaped_df)

# === Optional rule-based ice enhancement ===

# === Convert to integer-coded raster ===
label_map = {
    'water': 1,
    'ice': 0
}
label_map['ice'] = len(pipe.classes_)  # in case "Ice" was added via rule
